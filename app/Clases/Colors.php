...
class Colors
{
    ...
    public static function getColorgrid($colorrow, $contraste=false, $grade=0){

        if($contraste){
            $gridContrs;
            for($i=0; $i < count(self::$grid); $i++ ){
                $gridContrs[$i] = self::$grid[$i][$grade];
            }
            return $gridContrs;
        }
        return self::$grid[$colorrow];
    }

    public static function getColorsSelected($arrayColor){
        
        $arrayout = [];
        $indexs = [];
        $acm = 0;

        foreach($arrayColor as $colorselected){
            $arrayout[$acm] = self::$grid[$colorselected[0]][$colorselected[1]];
            $acm ++;
        }
